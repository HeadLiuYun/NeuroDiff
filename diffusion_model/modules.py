...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=3, out_channels=None, ani_stride=False):
        super().__init__()
        self.channels = channels
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
        self.dims = dims
        self.ani_stride = ani_stride
        if use_conv:
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            if self.ani_stride:
                x = F.interpolate(
                    x, (x.shape[2], x.shape[3] * 2, x.shape[4] * 2), mode="trilinear"
                )
            else:
                x = F.interpolate(
