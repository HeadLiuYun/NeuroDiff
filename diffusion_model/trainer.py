...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        if exists(betas):
            ...
            betas = cosine_beta_schedule(timesteps)  # 噪声强度

        alphas = 1. - betas  # 保留比例
        alphas_cumprod = np.cumprod(alphas, axis=0)  # alphas 的累积乘积。这个值在扩散过程中表示每个时间步的数据保留程度。
        alphas_cumprod_prev = np.append(1., alphas_cumprod[:-1])  # 前一个时间步累积值
        ...
        # register_buffer将这些常数注册为模型的缓冲区，这意味着它们将作为模型的一部分进行存储，并且在训练过程中不会被优化
...
class Trainer(object):
    def __init__(
        ...
        self.ds = dataset
        self.dl = cycle(
            data.DataLoader(self.ds, batch_size=train_batch_size, shuffle=True, num_workers=4, pin_memory=True))
