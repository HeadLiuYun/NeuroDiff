...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        stem = nn.Sequential(
            ...
            # nn.Conv3d(in_chans, dims[0], kernel_size=7, stride=(1, 2, 2), padding=3),
        )
...
class SegMamba(nn.Module):
    def __init__(
        ...
        self.vit = MambaEncoder(in_chans, depths=depths, dims=feat_size)
...
if __name__ == "__main__":
    ...
    net = SegMamba(in_chans=1,
                   ...
                   depths=[2, 2, 2, 2],
                   feat_size=[48, 96, 192, 384],
                   hidden_size=768).cuda()
    t1 = torch.rand(2, 1, 32, 128, 128).cuda()
    # print(net(t1).shape)

    from thop import profile

    # 计算 FLOPs 和参数量
    flops, params = profile(net, inputs=(t1,))
    print(f"FLOPs: {flops / 1e9} GFLOPs")  # 转为GFLOPs
    print(f"Params: {params / 1e6}M")  # 转为百万参数
    # print(net)
    # print(net(t1).shape)
    # 保存模型参数到ckpt文件中
