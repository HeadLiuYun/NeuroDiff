...
class NiftiPairImageGenerator(Dataset):
    def __init__(self,
        ...
        self.input_img = self.read_image(input_folder, pass_scaler=self.full_channel_mask)
        self.target_img = self.read_image(target_folder)
    ...
    def pair_file(self):
        ...
        pairs = []
        pairs.append((self.input_folder, self.target_folder))
        return pairs
    ...
    def read_image(self, file_path, pass_scaler=False):
        ...
        png_files = sorted([f for f in os.listdir(file_path) if f.endswith('.png')])
    ...
    def __getitem__(self, index):
        ...
        # input_img = self.read_image(input_file, pass_scaler=self.full_channel_mask)
        random_z = random.randint(0, self.input_img.shape[2] - self.depth_size)
        random_y = random.randint(0, self.input_img.shape[1] - self.input_size)
        random_x = random.randint(0, self.input_img.shape[0] - self.input_size)

        input_img = self.input_img[random_x:random_x + self.input_size, random_y:random_y + self.input_size,
                    random_z:random_z + self.depth_size].copy()
        # input_img = self.input_img[:, :, random_z:random_z + self.depth_size].copy()
        input_img = self.label2masks(input_img) if self.full_channel_mask else input_img
        input_img = self.resize_img(input_img) if not self.full_channel_mask else self.resize_img_4d(input_img)

        # target_img = self.read_image(target_file)
        # target_img = self.target_img[:, :, random_z:random_z + self.depth_size].copy()
        target_img = self.target_img[random_x:random_x + self.input_size, random_y:random_y + self.input_size,
                     random_z:random_z + self.depth_size].copy()
